...
#####
DO THIS NEXT PART START TO FINISH 5 TIMES
#####
...
getRF <- function(PVData, mergedData) {
  ...
  output <- list(correctRate1, correctRate2)
  return(output)
}
```
...
```{r}
getRF(coloPVData, coloMergedData)

getRF(epePVData, epeMergedData)

getRF(smokePVData, smokeMergedData)

getRF(lungPVData, lungMergedData)

getRF(colPVData, colMergedData)

getRF(fibPVData, fibMergedData)

getRF(intPVData, intMergedData)

getRF(desPVData, desMergedData)
